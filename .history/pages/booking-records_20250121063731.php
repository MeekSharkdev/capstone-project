...
<!-- Modal -->
<div id="viewModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-50">
    <div class="relative w-full max-w-3xl bg-white rounded-lg shadow-lg p-4 md:p-5">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5">
                ...
                <div class="mt-3">
                    ...
                    <select id="newPurposeInput" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                ...
                <div class="mt-3">
                    <label for="newPurposeInput" class="block text-sm font-medium text-gray-900 dark:text-white">Change Purpose:</label>
                    <input type="text" id="newPurposeInput" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                </div>
            </div>

            <!-- Modal footer -->
            <div class="flex justify-between p-4 md:p-5 border-t rounded-b dark:border-gray-600">
...
<script>
    ...
    document.querySelectorAll('.view-btn').forEach(button => {
        button.addEventListener('click', () => {
            ...
            document.getElementById('newPurposeInput').value = button.getAttribute('data-purpose'); // New purpose input
     ent.getElementById('confirmRescheduleBtn').addEventListener('click', () => {
    if (!selectedId) {
    ...
    const newPurpose = document.getElementById('newPurposeInput').value; // Get new purpose

    console.log(newDate, newPurpose);  // Add this line to debug the values

    if (newDate && newPurpose) {
        ...
        .then(data => {
            if (data.success) {
                alert('Schedule and purpose successfully updated!');
                document.getElementById('modalDate').textContent = newDate;
                document.getElementById('modalPurpose').textContent = newPurpose;
        ...
        alert('Please provide both a new date and purpose.');
    }
});

</script>       modal.classList.remove('hidden');  // Show modal
        });
    });

    // Event listener to handle reschedule and purpose change when the "Confirm Schedule" button is clicked
docum
