...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Insert the reschedule request into a new table (e.g., reschedules table)
    $query = "INSERT INTO reschedules (bookings_id, new_date, reason) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("iss", $bookings_id, $new_date, $reason);

    if ($stmt->execute()) {
        echo "<script>alert('Your reschedule request has been submitted.');</script>";
    } else {
...
<body class="bg-gray-200">
    ...
    <div class="max-w-7xl mx-auto bg-white p-8 rounded-lg shadow-xl mt-10 grid grid-cols-1 gap-8">
