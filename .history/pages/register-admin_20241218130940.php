...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $attachment_form_id = $_FILES['attachment_form_id']['name']; // Handle file upload

    // Insert into database
    $stmt = $conn->prepare("INSERT INTO users (firstname, lastname, phonenumber, email, birthdate, age, sex, role, password, attachment_form_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssss", $firstname, $lastname, $phonenumber, $email, $birthdate, $age, $sex, $role, $password, $attachment_form_id);

    if ($stmt->execute()) {
        move_uploaded_file($_FILES['attachment_form_id']['tmp_name'], "uploads/" . $attachment_form_id); // Handle file movement to uploads folder
