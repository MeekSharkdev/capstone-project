...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['email']) && isset($_POST['id'])) {
  // Assuming the post data for email, subject, and id is provided
  $email = $_POST['email'];
  $id = $_POST['id']; // Reschedule request ID

  // Fetch user details from the database
  try {
    $stmt = $pdo->prepare("SELECT firstname, lastname, purpose, date FROM request_reschedule WHERE id = :id");
...
<body class="bg-gray-200">
        ...
        <tbody>
          ...
          ?>
              <tr class="bg-white border-b hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                  <button class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600" onclick="openEmailModal('<?= $row['email']; ?>', <?= $row['id']; ?>)">
                    Send Email
                  </button>
                </td>
              </tr>
  ...
  <div id="emailModal" class="fixed inset-0 hidden bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg shadow-lg w-96">
      ...
      <form id="emailForm">
        ...
        <textarea id="emailMessage" class="w-full p-2 border rounded mb-4" rows="5" required></textarea>

        <!-- Modal Buttons -->
        <div class="flex justify-end">
          <button type="button" onclick="closeModal()" class="bg-gray-500 text-white px-4 py-2 rounded-md mr-2">Cancel</button>
          <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md">Send Email</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <script>
    ...
    function openEmailModal(email, id) {
      ...
      document.getElementById("emailSubject").value = "Reschedule Request Approved";
      ...
      document.getElementById("emailForm").onsubmit = function(event) {
        ...
        xhr.send("email=" + email + "&id=" + id + "&subject=" + encodeURIComponent(subject) + "&message=" + encodeURIComponent(message));
      };
    }
