...
$query = "
    SELECT 
        ...
        lastname,
        email,
        date, -- Using this for both original and rescheduled dates
        purpose,
        reason
    FROM 
        request_reschedule
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id'])) {
  ...
  try {
    $stmt = $pdo->prepare("SELECT firstname, lastname, email, purpose, date FROM request_reschedule WHERE id = :id");
    ...
    if ($user) {
      ...
      $date = $user['date']; // Use the `date` column for both dates
                      ...
                      <p>We are pleased to inform you that your request to reschedule the following service has been approved:</p>
                       <table style='border-collapse: collapse; width: 100%;'>
                          ...
                          <tr>
                              ...
                              <td>$date</td> <!-- Same date used for both original and rescheduled -->
                          </tr>
                      </table>
                       <br>
      ...
      if (sendApprovalEmail($email, $firstname, $lastname, $subject, $message)) {
        // Optionally handle further actions, but do not echo any output here
        // e.g., update database if necessary without redirect
      } else {
        // Optionally handle email failure here
        // e.g., log the failure or set a session variable for notification
      }
    } else {
      // Optionally handle request not found here
    }
  } catch (PDOException $e) {
