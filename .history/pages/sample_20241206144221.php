...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!preg_match('/^09\d{9}$/', $phonenumber)) {
        ...
        if ($bookings_count < 20) {
            $stmt = $conn->prepare("INSERT INTO bookings (booking_date, firstname, lastname, phonenumber, email, purpose) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssss", $booking_date, $firstname, $lastname, $phonenumber, $email, $purpose);
...
<script>
    function validatePhoneNumber() {
        ...
        // Show an alert if phone number doesn't start with "09" or isn't 11 digits
        if (phoneNumber.length > 11 || !/^09\d*$/.test(phoneNumber)) {
            phoneInput.setCustomValidity('Phone number must start with 09 and be exactly 11 digits.');
        } else {
