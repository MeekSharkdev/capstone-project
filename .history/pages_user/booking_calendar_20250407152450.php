    ...
    <div class="container mt-4">
        ...
        <div class="relative mb-6 max-w-lg mx-auto">
            ...
            <a href="?month=<?= $month == 1 ? 12 : $month - 1 ?>&year=<?= $month == 1 ? $year - 1 : $year ?>"
                class="absolute left-4 top-1/2 transform -translate-y-1/2 px-5 py-2.5 bg-blue-600 text-white font-medium rounded-full shadow-md hover:bg-blue-700 transition-all duration-300 ease-in-out">
                <i class="fas fa-chevron-left text-lg"></i>
            </a>

            <!-- Month and Year Header (Centered) -->
            <h3 class="text-xl font-semibold text-gray-700 text-center"><?= date('F Y', strtotime("$year-$month-01")) ?></h3>

            <!-- Next Button (Right) -->
            <a href="?month=<?= $month == 12 ? 1 : $month + 1 ?>&year=<?= $month == 12 ? $year + 1 : $year ?>"
                class="absolute right-4 top-1/2 transform -translate-y-1/2 px-5 py-2.5 bg-green-600 text-white font-medium rounded-full shadow-md hover:bg-green-700 transition-all duration-300 ease-in-out">
                <i class="fas fa-chevron-right text-lg"></i>
            </a>

            <!-- Centered Arrow Button for Sliding -->
            <a href="?month=<?= $month == 12 ? 1 : $month + 1 ?>&year=<?= $month == 12 ? $year + 1 : $year ?>"
                class="absolute top-4 left-1/2 transform -translate-x-1/2 p-4 bg-green-600 text-white font-semibold rounded-full shadow-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-300 ease-in-out">
