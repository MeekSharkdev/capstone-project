                ...
                <tbody>
                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <!-- Display event image -->
                                <?php if (!empty($row['event_image']) && file_exists('../images/' . $row['event_image'])): ?>
                                    <img src="../images/<?php echo htmlspecialchars($row['event_image']); ?>" alt="Event Image" class="w-32 h-32 object-cover">
                                <?php else: ?>
                                    No image available
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4">
                                ...
                                <a href="view_event_details.php?id=<?php echo $row['id']; ?>" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700">View Details</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
