...
require ('./config/mysqli_connect_req.php'); // Ensure this file correctly sets up $conn

// Check if bookings_id is provided in the URL
if (!isset($_GET['bookings_id']) || empty($_GET['bookings_id'])) {
    die("Bookings ID is missing. Please select a booking.");
}

$bookings_id = intval($_GET['bookings_id']); // Ensure it's a valid integer
...
if ($stmt = $conn->prepare($query)) {
    ...
    if ($result->num_rows > 0) {
        ...
        $middle_initial = $booking['middle_initial'] ?? ''; // Prevents errors if NULL
