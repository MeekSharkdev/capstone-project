        ...
        if ($user) {
            ...
            if ($entered_otp == $user['reset_token'] && time() <= $otp_expiry_time) {
                ...
                if ($new_password && $confirm_password && $new_password == $confirm_password) {
                    ...
                    $stmt->execute();
                    $password_changed = true; // Set flag to indicate password change is successful
...
<head>
    ...
    <script>
        ...
        function handleOtpInput(event) {
            ...
            // Automatically move to the next input if number is entered
...
<body class="flex items-center justify-center h-screen bg-gray-100">
    <div class="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
        ...
        <?php if (!$otp_valid && !$password_changed): ?>
            <!-- Show OTP fields only if OTP is not validated yet -->
        ...
        <?php elseif ($otp_valid && !$password_changed): ?>
            <!-- Show Password fields only if OTP is validated -->
            <form action="reset-password.php?email=<?php echo urlencode($email); ?>" method="POST" class="space-y-4">
        ...
        <?php else: ?>
            <!-- If password change was successful, you could show a message or redirect -->
            <p class="text-green-500 text-center mb-2">Password successfully changed. You are being redirected to the login page.</p>
        <?php endif; ?>
    </div>
</body>
