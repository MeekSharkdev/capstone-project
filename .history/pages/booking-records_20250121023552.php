...
<!-- Modal -->
<div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden fixed top-0 left-0 right-0 z-50 justify-center items-center w-full h-full bg-gray-800 bg-opacity-50">
    <div class="relative p-4 w-full max-w-md max-h-full bg-white rounded-lg shadow-lg">
        <div class="flex items-center justify-between p-4 border-b">
            ...
            <button type="button" class="text-gray-400 hover:bg-gray-200 hover:text-gray-900 rounded-lg p-2" data-modal-toggle="crud-modal">
                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                </svg>
            </button>
        ...
        <div class="p-4">
            ...
            <p><strong>Current Date:</strong> <span id="modalDate"></span></p>
            <div class="mt-3">
                <label for="newDateInput" class="block text-sm font-medium">Reschedule to:</label>
                <input type="date" id="newDateInput" class="form-control bg-gray-100 border border-gray-300 p-2 rounded-md">
            </div>
        </div>
...
// Event listener to handle reschedule confirmation
document.getElementById('confirmRescheduleBtn').addEventListener('click', () => {
    ...
    if (selectedId && newDate) {
        ...
        .then(data => {
            if (data.success) {
                alert('Schedule successfully rescheduled');
