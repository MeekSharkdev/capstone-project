...
<body>
    ...
    <!-- Modal -->
    <!-- Modal -->
    <div id="viewModal" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-50 opacity-0 transition-opacity duration-500 ease-in-out">
        <div class="relative p-4 w-full max-w-lg"> <!-- Adjusted width to max-w-lg for medium size -->
            <!-- Modal content -->
            <div class="bg-gray-700 rounded-lg shadow-2xl dark:bg-gray-700 transform scale-95 transition-transform duration-500 ease-in-out">
                <!-- Modal header -->
                <div class="flex items-center justify-between p-4 border-b dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                        Booking Details
                    </h3>
                    <button type="button" class="text-gray-100 bg-transparent hover:bg-gray-200 hover:text-gray-500 rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" id="closeModalBtn">
                ...
                <div class="p-9 text-white">
                    ...
                    <div class="mt-9">
                        ...
                        <input type="date" id="newDateInput" class="bg-blue border border-blue-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-md mt-3 p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-blue-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                    </div>
                </div>
                <!-- Modal footer -->
    ...
    <script>
        document.querySelectorAll('.confirm-btn').forEach(button => {
            button.addEventListener('click', () => {
                document.getElementById('bookingId').value = button.getAttribute('data-id');
            });
        });
    ...
    <script>
        ...
        // Modal open and close functionality with smooth slide-up animation only
        ...
        viewBtn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                modal.classList.add('transition-all', 'duration-500', 'opacity-100', 'translate-y-0'); // Smooth slide-up effect
            }, 10);
        });

        // Close modal when the close button is clicked (without animation)
        closeModalBtn.addEventListener('click', () => {
            modal.classList.add('hidden'); // Hide the modal immediately without animation
            modal.classList.remove('opacity-100', 'translate-y-0'); // Remove visible state
        });

        // Ensure the modal is hidden by default
        const modal = document.getElementById('viewModal'); // The modal element, make sure it has an ID 'viewModal'
        ...
        document.querySelectorAll('.view-btn').forEach(button => {
            button.addEventListener('click', () => {
                ...
                modal.classList.remove('hidden'); // Show the modal
                setTimeout(() => {
                    modal.classList.add('transition-all', 'duration-500', 'opacity-100', 'translate-y-0'); // Smooth animation
                }, 10);
            });
        });

        // Close modal if clicking outside or on the close button
        ...
        document.getElementById('confirmRescheduleBtn').addEventListener('click', () => {
            const selectedId = document.getElementById('selectedId').value; // Ensure that the selectedId is saved from the modal data
            ...
            const newDate = document.getElementById('newDateInput').value; // Get new date from input field
            if (newDate) {
