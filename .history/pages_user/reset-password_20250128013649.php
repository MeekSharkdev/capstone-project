...
<head>
    ...
    <script>
        ...
        function handleOtpInput(event) {
            ...
            // Validate for numeric input and update border accordingly
            if (/[^0-9]/.test(value)) {
                event.target.classList.add('border-red-500');
                event.target.classList.remove('border-gray-300', 'border-green-500');
            } else if (value.length === 1) {
                // If the input is numeric, check if it's correct
                if (value === correctOtp[index]) {
                    event.target.classList.add('border-green-500');
                    event.target.classList.remove('border-gray-300', 'border-red-500');
                } else {
                    event.target.classList.add('border-red-500');
                    event.target.classList.remove('border-gray-300', 'border-green-500');
                }
                ...
                event.target.classList.remove('border-gray-300', 'border-green-500', 'border-red-500');
            }
