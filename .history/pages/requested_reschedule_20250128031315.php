...
try {
  // Join request_reschedule with bookings to get booking_date
  $stmt = $pdo->prepare("
      SELECT rr.firstname, rr.lastname, rr.purpose, rr.date AS original_date, b.booking_date 
      FROM request_reschedule rr
      JOIN bookings b ON rr.id = b.request_reschedule_id
      WHERE rr.id = :id
  ");
  ...
  if ($stmt->execute()) {
    ...
    if ($user) {
      ...
      $original_date = $user['original_date'];
      $booking_date = $user['booking_date'];  // The new booking date from the bookings table
