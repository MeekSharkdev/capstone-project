...
$query = "SELECT booking_date, firstname, lastname, middle_initial, email FROM bookings ORDER BY booking_date DESC LIMIT 1";
$result = $conn->query($query);
$booking = $result->fetch_assoc();

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Check if $booking is not empty
    if ($booking) {
        // Get user input
        ...
        $stmt = $conn->prepare("INSERT INTO request_reschedule (firstname, middle_initial, lastname, email, date, reason) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $booking['firstname'], $booking['middle_initial'], $booking['lastname'], $booking['email'], $request_date, $reason);
        ...
        echo "<script>alert('No booking data found.');</script>";
    }
}
