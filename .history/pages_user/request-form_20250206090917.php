...
echo "User Email: " . $user_email;  // Debugging line to check session variable
...
if ($result->num_rows > 0) {
    ...
    echo "No booking found for this user.";  // Debugging line to check if booking exists
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt_reschedule = $conn->prepare("INSERT INTO request_reschedule (firstname, middle_initial, lastname, email, date, reason, purpose) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt_reschedule->bind_param("sssssss", $booking['firstname'], $booking['middle_initial'], $booking['lastname'], $booking['email'], $request_date, $reason, $booking['purpose']);

    if ($stmt_reschedule->execute()) {
