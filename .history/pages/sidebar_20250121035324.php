...
<aside id="default-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 bg-gray-800 text-white">
    <div class="h-full px-3 py-4 overflow-y-auto">
        ...
        <!-- Admin Panel Title -->
        <h1 class="text-center text-lg font-bold mb-9">Admin Panel</h1>

        <ul class="space-y-2">
            <li>
                <a href="./main_dashboard.php" class="flex items-center px-4 py-2 mt-9 hover:bg-gray-700 hover:scale-105 rounded transition-all duration-500">
        ...
        <!-- Display admin's first name at the bottom -->
        <div class="mt-auto text-center text-sm text-gray-300">
            <?php
                // Check if the session variable 'firstname' is set before displaying it
                if (isset($_SESSION['username'])) {
                    echo "Hi Admin, " . htmlspecialchars($_SESSION['username']);
                } else {
                    echo "Error po!";
                }
            ?>
