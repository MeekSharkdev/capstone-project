...
// Fetch search term from the form submission
$searchTerm = isset($_GET['search']) ? mysqli_real_escape_string($dbc, $_GET['search']) : '';

// Modify the query to filter users based on the search term
...
if ($searchTerm) {
    $query .= " WHERE firstname LIKE '%$searchTerm%' OR middlename LIKE '%$searchTerm%' OR lastname LIKE '%$searchTerm%' OR email LIKE '%$searchTerm%' OR username LIKE '%$searchTerm%'";
}
...
<body class="bg-gray-100">
    ...
    <div class="sm:ml-64 p-6">
        ...
        <!-- Search Bar -->
        <!-- Search Bar -->
<form action="manage_users.php" method="get" class="max-w-md mx-auto mb-6">
    ...
    <div class="relative">
        ...
        <input type="search" name="search" id="default-search" value="<?= htmlspecialchars($searchTerm) ?>" class="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search users..." />
