...
$errors = [];
$success = false; // Ensure success is initialized
...
$stmt->execute();
$stmt->bind_result($dbOtp, $isVerified);
$stmt->fetch();
$stmt->close();

if ($otp == $dbOtp) {
// OTP matches, update the account as verified
$stmt = $conn->prepare("UPDATE users SET otp = NULL, is_verified = 1 WHERE email = ?");
$stmt->bind_param("s", $email);
$stmt->execute();
$stmt->close();
$success = true;
        ...
        <?php if ($success): ?>
            <script>
                Swal.fire({
                    icon: 'success',
                    title: 'OTP Verified!',
                    text: 'Your OTP has been successfully verified. Your account is now verified.',
                    confirmButtonText: 'Proceed',
                    willClose: () => {
                        window.location.href = "../pages_user/login.php"; // Redirect after successful verification to login.php
                    }
                });
            </script>
        <?php endif; ?>
