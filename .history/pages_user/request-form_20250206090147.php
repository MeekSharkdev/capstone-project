...
$user_id = $_SESSION['id']; // Assuming 'id' is stored in the session
...
$query = "SELECT booking_date, firstname, lastname, middle_initial, email, purpose 
          FROM bookings 
          WHERE user_id = ?  -- Change to the correct column for user identification
          ORDER BY booking_date DESC 
          LIMIT 1";

$stmt = $conn->prepare($query);
$stmt->bind_param("i", $user_id); // "i" for an integer
$stmt->execute();
