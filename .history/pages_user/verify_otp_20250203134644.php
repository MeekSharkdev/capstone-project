...
if (empty($email)) {
    die("Invalid request. Email is required.");
}

// Function to fetch OTP and expiration time
function getOtpDetails($conn, $email)
{
    ...
    $stmt->fetch();
    $stmt->close();
    return [$otp_in_db, $otp_expiration_time];
}

list($otp_in_db, $otp_expiration_time) = getOtpDetails($conn, $email);
$current_time = time();
$expiration_time = strtotime($otp_expiration_time);

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['resend_otp'])) {
        // Generate new OTP and update expiration time
        $otp = rand(100000, 999999);
        $new_expiration_time = date("Y-m-d H:i:s", time() + 30); // 30 seconds expiration
...
<head>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            let expirationTime = new Date("<?php echo $otp_expiration_time; ?>").getTime();
