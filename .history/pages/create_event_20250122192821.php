...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Handle file upload
    $targetDir = $_SERVER['DOCUMENT_ROOT'] . '/Capstone/Scheduling_Appointment/uploads/events/';
    $imageName = basename($_FILES["event_image"]["name"]);
    $targetFile = $targetDir . $imageName;
    $imageFileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

    // Debug the paths
    echo "Temp Path: " . $_FILES["event_image"]["tmp_name"] . "<br>";
    echo "Target Path: " . $targetFile . "<br>";

    // Check for any upload errors
    if ($_FILES["event_image"]["error"] !== UPLOAD_ERR_OK) {
        $error_message = "Error during file upload. Code: " . $_FILES["event_image"]["error"];
    } else {
        // Check if image file is valid (JPG, JPEG, PNG, GIF)
