        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($entered_otp == $user['reset_token'] && time() <= $otp_expiry_time) {
                ...
                if ($new_password && $confirm_password && $new_password == $confirm_password) {
                    ...
                    $_SESSION['password_reset_success'] = "Password has been successfully updated!";

                    // Redirect to avoid resubmission of form data
                    header("Location: reset-password.php?email=" . urlencode($email) . "&reset=success");
                    exit;
...
<body class="flex items-center justify-center h-screen bg-gray-100">
    <div class="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
        ...
        if (isset($_GET['reset']) && $_GET['reset'] == 'success') {
            echo "<script>
                    swal('Success', 'Password has been successfully updated!', 'success').then(function() {
        ...
        if (isset($_SESSION['password_reset_success'])) {
            echo "<script>
                    swal('Success', '" . $_SESSION['password_reset_success'] . "', 'success').then(function() {
                        window.location = 'login.php'; 
                    });
                  </script>";
            unset($_SESSION['password_reset_success']);  // Clear session after showing the success message
        }
