...
// Debug: Check if email is stored in session correctly
echo "User Email from Session: " . htmlspecialchars($user_email);
...
$query = "SELECT booking_date, firstname, lastname, middle_initial, email, purpose 
          FROM bookings 
          WHERE email = ? 
          ORDER BY booking_date DESC";  // All bookings for user
...
if (empty($bookings)) {
    echo "No bookings found for user with email: $user_email";
}
?>
