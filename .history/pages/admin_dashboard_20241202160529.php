...
$query = "SELECT bookings.id, bookings.booking_date, users.firstname, users.email, bookings.purpose
          FROM bookings 
          INNER JOIN users ON bookings.user_id = users.id"; // Fixed join condition
...
// Debugging: Check how many rows are being returned and print the rows
echo "Total rows: " . mysqli_num_rows($result); // Output the number of rows

if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<pre>";
        print_r($row); // Output the contents of each row for debugging
        echo "</pre>";
    }
} else {
    echo "No bookings found."; // Message when no records are found
}
?>
