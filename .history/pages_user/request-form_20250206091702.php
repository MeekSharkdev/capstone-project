...
// Fetch booking details based on the logged-in user (assuming 'username' or 'id' is in session)
$username = $_SESSION['username']; // or $_SESSION['id'], depending on your login logic

// Query the bookings table for the current logged-in user
$query = "SELECT booking_date, firstname, lastname, middle_initial, email, purpose FROM bookings WHERE email = ? ORDER BY booking_date DESC LIMIT 1";
...
// If no booking found for this user, handle that case
if (!$booking) {
    echo "<script>alert('No booking found for this user.'); window.location.href='dashboard.php';</script>";
    exit();
}
