...
<body>
    ...
    <div class="container mt-4 px-4 sm:px-6 md:px-8">
        <h3 class="text-3xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold text-center text-gray-800 mb-6 border-b-4 border-blue-600 pb-2 tracking-wide leading-tight shadow-lg">
            Book Your Barangay Certificate Appointment
        </h3>

        <div class="flex justify-between items-center mb-6">
            <!-- Previous Button (Left) -->
            <a href="?month=<?= $month == 1 ? 12 : $month - 1 ?>&year=<?= $month == 1 ? $year - 1 : $year ?>"
                class="btn btn-secondary text-xs sm:text-sm md:text-base lg:text-lg px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2 rounded-lg shadow-md hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500">
                &laquo; Previous
            </a>

            <!-- Month and Year Header (Centered) -->
            <h3 class="text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold text-gray-700">
        ...
        <!-- Calendar Grid -->
        <div class="calendar grid grid-cols-7 gap-2 p-4 sm:grid-cols-7 md:grid-cols-7 lg:grid-cols-7">
            <?php
            $today = date('Y-m-d');
            ...
            foreach ($days_of_week as $day) {
                echo "<div class='day day-header text-center font-semibold text-sm sm:text-base md:text-base lg:text-lg text-gray-800'><strong>$day</strong></div>";
            }
            ...
            ?>
                <div class="day <?= $day_class ?> p-2 text-center rounded-lg cursor-pointer hover:bg-blue-100 text-sm sm:text-base md:text-base lg:text-lg" data-date="<?= $date ?>">
                    <strong class="block"><?= $day ?></strong>
                    <?php if ($is_holiday): ?>
                        <div class="holiday text-xs text-red-600"><?= $holiday_name ?></div>
                    <?php elseif (!$is_limit_reached): ?>
                        <div class="<?= $slots_left == 20 ? 'text-green-600' : '' ?> text-xs sm:text-sm md:text-sm lg:text-base">
