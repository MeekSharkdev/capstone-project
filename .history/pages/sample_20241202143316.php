...
$start_prev_month = "$prev_year-$prev_month-01";
$end_prev_month = "$prev_year-$prev_month-$days_in_prev_month";

// Fetch bookings for the current month
$start_date = "$year-$month-01";
$end_date = "$year-$month-$days_in_month";

// Fetch booking data for the current and previous month
$sql = "SELECT booking_date, COUNT(*) AS count FROM bookings WHERE (booking_date BETWEEN ? AND ?) OR (booking_date BETWEEN ? AND ?) GROUP BY booking_date";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ssss", $start_date, $end_date, $start_prev_month, $end_prev_month);
$stmt->execute();
$result = $stmt->get_result();
