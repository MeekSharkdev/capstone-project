...
// Prepare and execute the query to fetch the user's ID from the database
...
$user_id = $user['id'];
                                            ...
                                            // Fetch the latest booking details for the logged-in user using the username instead of user_id
                                            $query = "SELECT firstname, lastname, middle_initial, phonenumber, email, purpose, booking_date FROM bookings WHERE username = ? ORDER BY booking_date DESC LIMIT 1";// Assuming the users table contains 'id' and bookings table contains 'user_id' as foreign key
