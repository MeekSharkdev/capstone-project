...
<script>
    ...
    document.getElementById('confirmRescheduleBtn').addEventListener('click', () => {
        ...
        if (newDate && newPurpose) {
            fetch('update_booking_date_and_purpose.php', { // Adjust the backend file to handle both updates
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('modalPurpose').textContent = newPurpose; // Update purpose in modal
                    modal.classList.add('hidden'); // Close the modal after updating
