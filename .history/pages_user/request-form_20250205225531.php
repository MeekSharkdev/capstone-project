...
// Check if the user is logged in (you need to have a session variable set during login)
...
// Fetch the logged-in user's ID (you should have a session variable for user ID after login)
$user_id = $_SESSION['user_id']; // Assuming session variable for user ID is 'user_id'

// Fetch the logged-in user's latest booking details
$query = "SELECT booking_date, firstname, lastname, middle_initial, email, purpose FROM bookings WHERE user_id = ? ORDER BY booking_date DESC LIMIT 1";
...
// If the user doesn't have a booking yet, handle that case
if (!$booking) {
    echo "<script>alert('No booking found for your account.'); window.location.href='booking-calendar.php';</script>";
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Reschedule request submitted successfully!');</script>";
        // Don't redirect after form submission, stay on the same page
    } else {
