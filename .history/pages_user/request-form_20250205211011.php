...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt->bind_param("sssssss", $booking['firstname'], $booking['middle_initial'], $booking['lastname'], $booking['phonenumber'], $booking['email'], $date, $reason);
...
<body class="bg-gray-200 h-screen flex items-center justify-center">
    <div class="w-full max-w-md bg-white p-8 rounded-lg shadow-xl">
        ...
        <form action="" method="POST">
            ...
            <input type="text" name="firstname" value="<?php echo htmlspecialchars($bookings['firstname'] ?? ''); ?>" readonly class="w-full px-4 py-2 border rounded-lg mb-4">

            <label class="block font-medium">Middle Initial</label>
            <input type="text" name="middle_initial" value="<?php echo htmlspecialchars($bookings['middle_initial'] ?? ''); ?>" readonly class="w-full px-4 py-2 border rounded-lg mb-4">

            <label class="block font-medium">Last Name</label>
            <input type="text" name="lastname" value="<?php echo htmlspecialchars($bookings['lastname'] ?? ''); ?>" readonly class="w-full px-4 py-2 border rounded-lg mb-4">

            <label class="block font-medium">Phone Number</label>
            <input type="text" name="phonenumber" value="<?php echo htmlspecialchars($bookings['phonenumber'] ?? ''); ?>" readonly class="w-full px-4 py-2 border rounded-lg mb-4">

            <label class="block font-medium">Email Address</label>
            <input type="email" name="email" value="<?php echo htmlspecialchars($bookings['email'] ?? ''); ?>" readonly class="w-full px-4 py-2 border rounded-lg mb-4">
